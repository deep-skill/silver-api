...
const update = async (
  ...
  if(driverRating) {
    ...
    if (trip.driverRating == null) {
      driver.rating = (driver.rating * driver.doneTrips + driverRating) / (driver.doneTrips + 1);
      driver.doneTrips += 1;
      trip.driverRating = driverRating;
    } else {
      driver.rating = (driver.rating * driver.doneTrips - trip.driverRating + driverRating) / (driver.doneTrips);
      trip.driverRating = driverRating;
    }
    await driver.save();
  } 
  if(passengerRating) {
    ...
    if (trip.passengerRating == null) {
      user.rating = (user.rating * user.doneTrips + passengerRating) / (user.doneTrips + 1);
      user.doneTrips += 1;
      trip.passengerRating = passengerRating;
    } else {
      user.rating = (user.rating * user.doneTrips - trip.driverRating + passengerRating) / (user.doneTrips);
      trip.passengerRating = passengerRating;
    }
    await user.save();
  } 
